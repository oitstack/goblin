...
public class HttpWaitStrategy extends AbstractWaitStrategy {
    ...
    protected void waitUntilReady() {
        ...
        final Integer livenessCheckPort = livenessPort.map(runtime::getPortByInnerPort).orElseGet(() -> {
            final Set<Integer> livenessCheckPorts = getLivenessCheckPorts();
            if (livenessCheckPorts == null || livenessCheckPorts.isEmpty()) {
                log.warn("{}: No exposed ports or mapped ports - cannot wait for status", runtime.getImageName());
                return -1;
            }
        ...
        try {
            ...
            Integer originalPort = getOriginalPort(rawUri,runtime.getExposedPorts());
            log.info("{}: Waiting for {} seconds for URL: {} (where port {} maps to container port {})", runtime.getImageName(), startupTimeout.getSeconds(), uri, rawUri.getPort(), originalPort);
        ...
        try {
            RetryUtils.retryUntilTrue((int) startupTimeout.getSeconds(), TimeUnit.SECONDS, () -> {
                return triggerUri(uri);
            });
    ...
    private Integer getOriginalPort(URI rawUri, List<Integer> exposedPorts) {
        return exposedPorts.stream()
                .filter(it -> rawUri.getPort() == runtime.getPortByInnerPort(it))
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Target port " + rawUri.getPort() + " is not exposed"));
    }

    private boolean triggerUri(String uri) {
        try {
    ...
    private URI buildLivenessUri(int livenessCheckPort) {
        ...
        final String host = runtime.getHost();
